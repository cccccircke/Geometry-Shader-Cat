...
unsigned int  shaderProgram,program1, pro, progn;
int main()
{
	// Initialization
	glfwInit();
	...
	vert = createShader("shaders/new1.vert", "vert");
	geom = createShader("shaders/new1.geom", "geom");
	frag = createShader("shaders/new1.frag", "frag");
	program1 = createProgram(vert, geom, frag);
	vert = createShader("shaders/ne.vert", "vert");
	geom = createShader("shaders/ne.geom", "geom");
	frag = createShader("shaders/ne.frag", "frag");
	pro = createProgram(vert, geom, frag);
	vert = createShader("shaders/new.vert", "vert");
	geom = createShader("shaders/new.geom", "geom");
	frag = createShader("shaders/new.frag", "frag");
	progn = createProgram(vert, geom, frag);
	// Texture
...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if ((key == GLFW_KEY_2 && action == GLFW_PRESS) || (key == GLFW_KEY_KP_2 && action == GLFW_PRESS)) {
		program = pro;
		cout << "You press 2" << endl;
	}

	if ((key == GLFW_KEY_3 && action == GLFW_PRESS) || (key == GLFW_KEY_KP_3 && action == GLFW_PRESS)) {
		program = program1;
